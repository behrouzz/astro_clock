    ...
    def my_function(self, i):
        ...
        if (self.kind is not None) and (self.lon is not None):
            ...
            if self.kind=='true_solar':
                ...
                t = c.lst
            self.title = self.kind.upper().replace('_', ' ') + ' TIME'
        ...
        if self.title is not None:
            title_angles = np.linspace(-60, 60, len(self.title))
            for ii, val in enumerate(self.title):
                self.ax.text(
                    x=title_angles[ii]*(np.pi/180),
                    y=0.6,
                    rotation=360-title_angles[ii],
                    s=val,
                    #fontfamily='fantasy',
                    #fontsize='small',
                    c = 'gray',
